...
struct mtsclientglobal
{
    ...
    virtual void load_lib()
    {
       if (getenv("MTS_LIB_LOCATION")) {
           std::cout << "Client Overriding MTS Location with [" << getenv("MTS_LIB_LOCATION") << "]" << std::endl;
          handle = dlopen(getenv("MTS_LIB_LOCATION"), RTLD_NOW);
       }
        if (!handle && !(handle=dlopen("/Library/Application Support/MTS-ESP/libMTS.dylib",RTLD_NOW)) &&
